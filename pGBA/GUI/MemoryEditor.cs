...
namespace pGBA
{
	...
	public partial class MemoryEditor : Form
	{
		...
		void RefreshMem()
		{
			...
			g.DrawString("0123456789", font, Brushes.White, new PointF(0, 0));
		}
		...
		{
  			m_mouseHeldPoint = memText.PointToScreen(e.Location);
		}
		
		private void memText_MouseUp(object sender, MouseEventArgs e)
		{
			g.Clear(Color.DarkGray);
  			Point currentPoint = memText.PointToScreen(e.Location);
  			int xFinal = e.Location.X / Convert.ToInt32(font.SizeInPoints);
  			int yFinal = e.Location.Y / Convert.ToInt32(font.GetHeight());
  			
  			xFinal *= Convert.ToInt32(font.SizeInPoints);
  			yFinal *= Convert.ToInt32(font.GetHeight());
  			
  			//Pen p = new Pen(Color.Red);
  			Rectangle rect = new Rectangle(2+xFinal,3+yFinal,8,8);
			//g.DrawRectangle(p, xFinal, yFinal, 9, 9);
			g.FillRectangle(Brushes.Blue, rect);
		}
		
		void MemTextPaint(object sender, PaintEventArgs e)
		{
			RefreshMem();
		}
	}
